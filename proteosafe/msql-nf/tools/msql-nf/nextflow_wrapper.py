...
def main():
    ...
    if args.runcluster == "YES" and args.user in ["mwang87"]:
        cmd = "source {} {} && nextflow run {} -c {} -with-trace -with-dag -with-report report.html".format(args.conda_activate, args.nextflow_conda_environment,
                            args.nextflow_script, args.clusterconfig)
    else:
        cmd = "source {} {} && nextflow run {} -with-trace -with-dag -with-report report.html".format(args.conda_activate, args.nextflow_conda_environment,
